...
function MapaTalhoes({ onAreaMedida }) {
  ...
  useEffect(() => {
    // espera o leaflet carregar antes de inicializar
    function carregarLeaflet() {
      ...
      script.onload = () => {
        // pequeno delay pra garantir que o div tá na tela
  ...
  function inicializarMapa() {
    ...
    // tenta ir pra localização do usuário
  ...
  function adicionarPonto(e) {
    ...
    const marcador = window.L.circleMarker([novoPonto.lat, novoPonto.lng], {
      radius: 6, color: "#2d6a4f", fillColor: "#4ade80", fillOpacity: 1, weight: 2,
    ...
    if (pontosRef.current.length > 1) {
      linhaRef.current = window.L.polyline(
        ...
        { color: "#2d6a4f", weight: 2, dashArray: "5, 5" }
      ).addTo(mapaInstancia.current);
    }
  }
  ...
  function finalizarDesenho() {
    ...
    poligonoRef.current = window.L.polygon(
      pontosRef.current.map((p) => [p.lat, p.lng]),
      { color: "#2d6a4f", fillColor: "#4ade80", fillOpacity: 0.3, weight: 2 }
    ).addTo(mapaInstancia.current);

    mapaInstancia.current.off("click", adicionarPonto);
    setDesenhando(false);
    onAreaMedida(calcularAreaHectares(pontosRef.current));
  }
        ...
        {!desenhando ? (
          <div className="mapa-controles-normal">
            ...
            <button
              ...
              disabled={carregandoLoc || !mapaProto}
            >
          ...
          <div className="mapa-controles-desenho">
            ...
            {pontos.length >= 3 && (
              <button className="btn-finalizar" onClick={finalizarDesenho}>
                Finalizar talhão
              </button>
            )}
            <button className="btn-cancelar-mapa" onClick={cancelarDesenho}>
              Cancelar
            </button>
          </div>
        )}
